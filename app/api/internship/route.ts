...
function buildMessage(fields: Record<string, string>, resumeFilename: string) {
  return [
    "New Enlivo internship application received.",
    "",
    ...
    "",
    `Resume filename: ${resumeFilename || "Not provided"}`,
  ].join("\n");
}

function buildHtml(fields: Record<string, string>, resumeFilename: string) {
  const rows = [
    ["Full name", fields.fullName],
    ["Email", fields.email],
    ["Phone", fields.phone],
  ...
  return `
    <div style="font-family: Inter, Arial, sans-serif; color: #111827;">
      <h2>New Enlivo internship application</h2>
...
export async function POST(request: Request) {
  try {
    ...
    const resumeFilename = resume instanceof File ? resume.name || "resume" : typeof resume === "string" ? resume : "";
    ...
    const { error } = await resend.emails.send({
      ...
      subject: "New Enlivo Internship Application",
      text: buildMessage(fields, resumeFilename),
      html: buildHtml(fields, resumeFilename),
    });

    if (error) {
      console.error("Internship email submission failed:", error);
    ...
    return Response.json({ ok: true, resumeFilename: resumeFilename || null });
  } catch (error) {
    console.error("Internship submission route failed:", error);
